...
class DataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        train_ds = AVDataset(
            root=ds_args.root,
            label_path=os.path.join(ds_args.root, ds_args.label_dir, ds_args.train_file),
            subset="train",
            modality=self.cfg.data.modality,
            audio_transform=AudioTransform("train"),
            video_transform=VideoTransform("train"),
        )
        if self.cfg.data.multi_lang:
            sampler = ByFrameCountSamplerMultilang(train_ds, self.cfg.data.max_frames, max_language_token=self.cfg.data.max_language_token)
        else:
            sampler = ByFrameCountSampler(train_ds, self.cfg.data.max_frames)
        if self.total_gpus > 1:
            sampler = DistributedSamplerWrapper(sampler, num_replicas=self.total_gpus, rank=int(os.environ["RANK"]))
        else:
            sampler = RandomSamplerWrapper(sampler)
        
        ret_dataloader = self._dataloader(train_ds, sampler, collate_pad)
        
        return ret_dataloader
