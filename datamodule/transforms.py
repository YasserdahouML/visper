...
class TextTransform:
    ...
    def __init__(
        ...
        self.hashmap = {unit.split()[0]: str(i+1) for i,unit in enumerate(units)}
    ...
    def tokenize(self, text, language=''):
        if language == '<zh>': 
            tokens = self.spm.EncodeAsPieces(' '.join([i for i in text]))
            tokens = tokens[0:1] + [i for i in tokens[1:] if i != '▁']
        else:
            tokens = self.spm.EncodeAsPieces(text)
        token_ids = [self.hashmap.get(token, self.hashmap["<unk>"]) for token in tokens]
        if len(language) > 0:
            token_lg = self.hashmap[language]
            token_ids = list(token_lg)+token_ids
