...
def remove_characters(text, lang='<fr>'):
    chars_to_remove = ['≥', '▁', '_','■','「','」','√', '‐','⁄','{', '}', ']', '[', '٠٠٠', '[\\p{Cf}]','\u200B', 'ـ', '¡', '🧐', '❤️', '://', ')', '¿' , '♪♪', '♪','(' ,'⁄' , '...', '****', '***', '**','?', '!', '.', ',', '+', '-', '...', '°', '»', '–', '%', '«', '»', '$', '€', '"', '&', '...', '—', '▁']
    chars_to_remove += ['...', '*', '/', '=' ':','|', '>', '<', '▁', '×', '؇', '٪', '،',]
    ...
    text = remove_japanese_and_korean(remove_non_zero_width_whitespace(text))
    text = remove_non_zero_width_whitespace(text)
    text = retain_characters(text)
...
def remove_successive_repeats(text):
    # Use regular expression to replace repeated characters with a single occurrence
    cleaned_text = re.sub(r'(.)\1{3,}', r'\1', text)
    return cleaned_text

def retain_characters(text):
    # Regular expression pattern to match Arabic, English, French, Spanish and Chinese characters
    pattern = r"[^\u0600-\u06FFa-zA-Z\s'0-9\u0080-\u00FF\u0100-\u017F\u4E00-\u9FFF]+"
    # Match the characters using the pattern
...
def remove_japanese_and_korean(text):
    # Define a regular expression pattern to match Japanese and Korean characters
    pattern = re.compile("[\u3040-\u309F\u30A0-\u30FF\u3131-\u318E\uAC00-\uD7A3]+")

    # Replace Japanese and Korean characters with an empty string
    cleaned_text = re.sub(pattern, "", text)
    
    return cleaned_text


def remove_non_zero_width_whitespace(text):
    # Define a regular expression pattern to match non-zero width whitespace characters
    whitespace_pattern = re.compile("[\u200B-\u200D\u2028-\u202E\u2060-\u2064\uFEFF]+")
    
    # Replace non-zero width whitespace characters with an empty string
