...
class BatchBeamSearch(BeamSearch):
    ...
    def init_hyp(self, x: torch.Tensor, lang=None) -> BatchHypothesis:
        ...
        return self.batchfy(
            [
                Hypothesis(
                    ...
                    scores=init_scores,
                    states=init_states,
                    yseq=torch.tensor([self.sos, lang], device=x.device) if lang is not None else torch.tensor([self.sos], device=x.device),
                )
            ]
        )
